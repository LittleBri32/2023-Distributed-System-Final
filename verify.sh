...
run_log "Test service: localhost:8080/service/3"
responds_with \
    "Hello from behind Envoy (service 3)!" \
...
run_log "Test service: https://localhost:8443/service/3"
responds_with \
    "Hello from behind Envoy (service 3)!" \
    -k https://localhost:8443/service/3

run_log "Scale up docker service3=3"
docker-compose scale service3=3
...
docker-compose exec -T front-envoy bash -c "\
                   ...
                   && curl -s http://localhost:8080/service/3 \
                   && curl -s http://localhost:8080/service/3" \
                   | grep Hello | grep "service 3"


run_log "Test service inside front-envoy: localhost:8080/service/3"
docker-compose exec -T front-envoy curl -s http://localhost:8080/service/3 | grep Hello | grep "service 3"
